// GuitarAMI Module - Main file
// Edu Meneses
// IDMIL - McGill University (2020)

const unsigned int firmware_version = 210518;
...
void setup() {
  ...
  // Define this module full name
    snprintf(global.deviceName,(sizeof(global.deviceName)-1),"GuitarAMI_module_%03i",settings.id);
  ...
  if (SPIFFS.exists("/config.json")) { // file exists, reading and loading
    ...
    DeserializationError error = deserializeJson(doc, configFile);
      if (error)
        ...
        Serial.println("GuitarAMI module configuration file loaded.\n");
      }
    } 
  else
...
void initWebServer() {
    ...
    server.on("/", HTTP_POST, [](AsyncWebServerRequest *request) {
      ...
      if(request->hasParam("reboot", true)) {
        ...
        if(request->hasParam("mode", true)) {
            ...
            settings.oldMode = settings.mode;
            printf("GuitarAMI mode stored: %d\n", settings.mode);
        } else {
  ...
  // Route to documentation
    server.on("/doc", HTTP_GET, [](AsyncWebServerRequest *request) {
      request->send(SPIFFS, "/docs/GuitarAMI_module.html");
    });
    server.on("/GuitarAMI_module.md", HTTP_GET, [](AsyncWebServerRequest *request) {
      request->send(SPIFFS, "/docs/GuitarAMI_module.md", String(), true);
    });
    server.on("/GuitarAMI_module.pd", HTTP_GET, [](AsyncWebServerRequest *request) {
      request->send(SPIFFS, "/docs/GuitarAMI_module.pd", String(), true);
    });
...
void sendInfo(OSCMessage &msg) {
  ...
  snprintf(namespaceBuffer,(sizeof(namespaceBuffer)-1),"/GuitarAMI_%i/info",settings.id);
  IPAddress oscIP;  
